...
BEGIN
  SELECT 
       ...
       CONCAT(a.au_nombre, ' ', a.au_apellido) AS nombre_autor, 
        ma.descripcion
...
-- Mostrar Report
DELIMITER //
CREATE PROCEDURE procSelectReportMaterial_Autores()
BEGIN
 SELECT 
      CONCAT(a.au_nombre, ' ', a.au_apellido) AS nombre_autor,
      GROUP_CONCAT(ma.id_material_autores SEPARATOR ', ') AS ids_material_autores,
      GROUP_CONCAT(t.mat_titulo SEPARATOR ', ') AS materiales,
      GROUP_CONCAT(ma.descripcion SEPARATOR ' | ') AS descripciones
  FROM tbl_material_edu_has_tbl_autores ma
  INNER JOIN tbl_material_edu t ON ma.tbl_material_edu_mat_id = t.mat_id
  INNER JOIN tbl_autores a ON ma.tbl_autores_au_id = a.au_id
